...
<?php
      ...
      $sql = "SELECT DISTINCT A.idArea as aid, A.name as name FROM Areas as A, Researches as R, ResearchMembers as RM WHERE R.idResearch = RM.idResearch and RM.idUser = $id and R.idArea = A.idArea UNION
              SELECT DISTINCT A.idArea as aid, A.name as name FROM Areas as A, Researches as R, ResearchMembers as RM, 
SectionMembers as SM, Sections as S WHERE R.idResearch = S.idResearch and S.idSection = SM.idSection and SM.idUser = $id and R.idArea = A.idArea ORDER BY name";
      $exe = mysql_query( $sql, $myrmconn) or print(mysql_error());
      if($exe != null)
      {
   ...
   if($exe != null)
   {
       while($line = mysql_fetch_array($exe)) // should be while!
       {
           ...
           if($uid != 0)
           {
              ...
              $what  = preg_replace("/(uid)=($uid)/i", "<a href=\"user.php?uid=$uid\"><b>User:</b>$u_name</a>", $what); // sid
           }
           ...
           if($fid != 0)
           {
              $filename = getFilenameByFileId($fid);
              $what  = preg_replace("/(fid)=($fid)/i", "<b>File:</b><i>\"$filename\"</i>", $what); // fid
           }


           $pid   = preg_replace('/(.*)(pid)=(\d+)(.*)/i', '${3}', $what); // pid
           $pid++; $pid--; // test number
           if($pid != 0)
           {
              $title = getTitleByPublicationId($pid);
              $what  = preg_replace("/(pid)=($pid)/i", "<b>Publication:</b><i>\"$title\"</i>", $what); // pid
           }
           ...
           if($sid != 0)
           {
              ...
              $what  = preg_replace("/(sid)=($sid)/i", "<b>Section:</b><a href=\"myrm.php#s$sid\">$s_name</a>", $what); // sid
           }
           ...
           if($rid != 0)
           {
              ...
              $what  = preg_replace("/(rid)=($rid)/i", "<b>Research:</b><a href=\"myrm.php#r$rid\">$r_name</a>", $what); // rid
           }
           ...
           if($aid != 0)
           {
              ...
              $what  = preg_replace("/(aid)=($aid)/i", "<b>Area:</b><a href=\"myrm.php#a$aid\">$a_name</a>", $what); // rid
           }
           echo "<td>$what</td>";
           echo "</tr>";
       }
